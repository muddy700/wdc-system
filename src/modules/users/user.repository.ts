...
export const createUserWithDBTransaction = async (body: IUser, session: ClientSession) => {
...
export const getCompanyUsers = async (offset: number, perPage: number, searchQuery: string, role: string = '') => {
    try {
        let condition: any = {};
        ...
        const users = await User.aggregate([
            ...
            },
            {
                $match: {
                    ...
                    ...condition,
                    // ...condition, type: USER_TYPES.STAFF
                },
            },
            { $sort: { firstName: -1 } },
            {
...
export const updateUserWithSession = async (userId: string, body: IUser, session: ClientSession) => {
    const user = await User.findOneAndUpdate({ _id: userId }, { ...body }, { new: true, session })
        .populate('role');
...
export const getUserByEmail = async (email: string, type: any) => {
    const userType: any = { type };
...
export const newUsers = async (startDate: Date, endDate: Date, country: string) => {
    try {
        let condition: any = {};
        ...
        const newUsers = await User.find({ ...condition, type: USER_TYPES.USER, createdAt: { $gt: endDate, $lte: startDate } })
            .sort({ createdAt: -1 });
...
export const userCount = (fromDate: Date, toDate: Date, country: string) => {
    ...
    return User.countDocuments({ type: USER_TYPES.USER, country, createdAt: { $gte: fromDate, $lte: new Date(toDate.setDate(toDate.getDate() + 1)) } });
}